...
fun main() {
    ...
    // Convert the hexadecimal public key to a byte array
    val pubKeyBytes = pubKeyA.HexToByteArray()

    // Choose a human-readable prefix (HRP)
    val hrp = "npub"

    // Encode the public key using Bech32
    val bech32Data = pubKeyBytes.toBech32Data(hrp)

    // Print the Bech32 encoded address
    val nsec = bech32Data.address
    println(nsec)

    val priv = Bech32.decode(nsec).data.ByteArrayToHex()
    println(priv)
